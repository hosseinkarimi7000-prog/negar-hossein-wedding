...
(() => {
  ...
  gate.dataset.coverMotion='initializing';
  ...
  if(!ctx){gate.dataset.coverMotion='unavailable';canvas.remove();return;}
  ...
  function sync(){
    ...
    canvas.dataset.motion=running?'running':'paused';gate.dataset.coverMotion=canvas.dataset.motion;
  ...
  photo.decode().then(()=>{ready=true;resize();sync();}).catch(()=>{stopped=true;sync();gate.dataset.coverMotion='image-unavailable';canvas.remove();});
})();
