(function(root){
  ...
  function response(invite,attendance,count,extras){
    ...
    if(!Number.isInteger(confirmed)||confirmed<0||(attendance==='yes'&&confirmed<1)||confirmed>(personal?invite.seats*2:1))throw Error('تعداد همراهان معتبر نیست.');
    if(extras.length!==Math.max(0,confirmed-(personal?invite.seats:1))||extras.some(p=>!text(p.name)||!text(p.relationship)))throw Error('نام و نام خانوادگی و نسبت هر همراه اضافه را کامل کنید.');
